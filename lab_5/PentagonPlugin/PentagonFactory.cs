...
namespace PentagonPlugin
{
    public class PentagonFactory : IShapeFactoryPlugin
    {
        ...
        /// <summary>
        /// Serialize Pentagon shape to JSON string
        /// </summary>
        public string SerializeShape(object shape)
        {
            if (shape is Pentagon pentagon)
            {
                var data = new { Radius = pentagon.Radius };
        ...
        /// <summary>
        /// Deserialize Pentagon shape from JSON string
        /// </summary>
        public object DeserializeShape(string data, int defaultX, int defaultY)
        {
            try
            {
                var dto = JsonSerializer.Deserialize<PentagonDto>(data);
                return new Pentagon(defaultX, defaultY, dto?.Radius ?? 60);
            }
            catch
            {
                return new Pentagon(defaultX, defaultY, 60);
            }
        }

        private class PentagonDto
        {
